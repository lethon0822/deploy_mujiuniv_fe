        ...
        <tr v-for="(row, ri) in matrix" :key="ri">
          ...
          >
            ...
            <div class="dots" v-if="typesFor(d).length">
              ...
              />
              <span v-if="typesFor(d).length > 3" class="more"
                >+{{ typesFor(d).length - 3 }}</span
              >
            </div>
          </td>
        </tr>
...
.day.selected {
  background: #eef5ff;
  box-shadow: inset 0 0 0 1.5px #bfd9ff, 0 2px 0 rgba(0, 0, 0, 0.02);
}
.day.selected:hover {
  background: #e8f1ff;
}
...
.day.selected.today {
  ...
  box-shadow: inset 0 0 0 2px #7bb9ff, 0 2px 0 rgba(0, 0, 0, 0.02);
}
